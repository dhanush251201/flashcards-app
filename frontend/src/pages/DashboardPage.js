...
export const DashboardPage = () => {
    ...
    const { data: activityData = [] } = useQuery({
        queryKey: ["activity"],
        queryFn: async () => {
            const { data } = await apiClient.get("/study/activity?days=7");
            return data;
        }
    });
