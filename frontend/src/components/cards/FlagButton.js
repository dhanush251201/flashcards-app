...
export const FlagButton = ({ cardId, deckId, isFlagged, onFlagChange, size = "md", showLabel = false }) => {
    ...
    return (_jsxs("button", { onClick: handleClick, disabled: isLoading, className: `
        ${sizeClasses[size]}
      ...
      `, title: isFlagged ? "Unflag this card" : "Flag this card for review", "aria-label": isFlagged ? "Unflag card" : "Flag card", children: [_jsx(Icon, { className: iconSizeClasses[size] }), showLabel && (_jsx("span", { className: "text-sm font-medium", children: isFlagged ? "Flagged" : "Flag" }))] }));
};
